...
object Day23Part1 extends IOApp {
  ...
  def isStupidMove(pod: Amphipod, newPos: Point, otherPods: Set[Day23Part1.Amphipod], podsWithFreeWay: Set[Amphipod], grid: Grid[Char]): Boolean = {
    ...
    (inOwnRoom(pod, pod.position) && newPos.y == 2) ||
    // don't move anywhere else than to bottom of own room if next to it and it is free
      (inOwnRoom(pod, pod.position) && pod.position.y == 2 && ownRoomSafe(pod, otherPods) &&
        !(newPos.y == 3 && newPos.x == pod.position.x)) ||
    // don't move out of own home when home is full with own pods
      (inOwnRoom(pod, pod.position) && {
        val brother = otherPods.find(_.kind == pod.kind).get
        inOwnRoom(brother, brother.position)
      }) || {
...
//    println()
    ...
    val rawSolutions = pods.flatMap { pod  =>
      ...
      }.filterNot { p => isStupidMove(pod, p, otherPods, podsWithFreeWay, grid) }
        .map { p =>
...
//      printState(grid, next.pods)
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    for {
      sc <- scannerFromResource("/day23.txt")
